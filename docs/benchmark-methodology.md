...
| B11 | WAL group commit + read-confirm amortization | Profile-driven fix for the two measured bottlenecks (WAL fsync serialization; per-GET Raft quorum round trip) | Linux/Docker, threadpool (8 workers), 8 clients. Baseline profile identified both bottlenecks by direct measurement (see [b11-baseline-2026-08-24.txt](benchmarks/results/b11-baseline-2026-08-24.txt)). Group commit: single-node write-heavy (ratio 20) throughput **3.01x** (4,570 → 13,763 ops/sec), and the write-path scaling check flips from flat/declining with concurrency to actually scaling (**3.4x–3.65x** at 8–32 clients) — [b11-group-commit-2026-08-24.txt](benchmarks/results/b11-group-commit-2026-08-24.txt). Read-confirm amortization: read-only Raft-leader throughput **12.2x** (12,102 → 151,044 ops/sec), 80/20 mix **1.53x** with p50 **20.6x** lower — [b11-read-confirm-2026-08-24.txt](benchmarks/results/b11-read-confirm-2026-08-24.txt). Connection-reuse (Part D) evaluated and not implemented; see [performance-notes.md](performance-notes.md) for why. |

...
|---|---|---|---|
| Storage ceiling | In-memory map, no I/O | Storage-layer throughput ceiling | Set ~4.9M/s, Get ~4.5M/s, mixed 80/20 ~5.0M/s (single-threaded); sharded-lock concurrency: 1 thread 32.9M/s → 4 threads 8.0M/s → 8 threads 4.6M/s → 16 threads 2.0M/s. Raw: [storage-throughput-2026-08-23.csv](benchmarks/results/storage-throughput-2026-08-23.csv) |
| Blocking TCP | Blocking TCP, single thread | Syscall-bound baseline | 1 client, 20s, 80/20 mix: 72,746 ops/sec, p50=13µs p95=18µs p99=22µs. Raw: [blocking-tcp-2026-08-23.txt](benchmarks/results/blocking-tcp-2026-08-23.txt) |
| Thread pool | Thread-pool TCP | Concurrency vs. blocking TCP | |
| Sharded storage | Sharded storage | Lock-contention reduction vs. thread pool | |
| Epoll | Epoll event loop | Event-driven I/O vs. thread pool | |
| WAL durability | + WAL fsync | Durability cost on top of epoll | Linux/Docker, 30s, 80/20 mix, WAL always on: epoll (1 thread) 8 clients 10,135 ops/sec (p50=52µs p95=3412µs p99=4118µs); thread-pool (8 workers) 8 clients 14,263 ops/sec (p50=30µs p95=3094µs p99=3826µs); thread-pool (64 workers) 64 clients 9,145 ops/sec (p50=35µs p95=18,273µs p99=21,416µs) — write throughput is capped by one fsync at a time regardless of thread count (DurableStorage serializes the whole log with one mutex), so more workers mainly buys headroom for concurrent GETs, not more concurrent SETs; tail latency grows with contention on that same lock. Raw: [wal-durability-2026-08-23.txt](benchmarks/results/wal-durability-2026-08-23.txt) |
| Raft replication | 3-node Raft | Consensus overhead vs. single-node WAL durability | Linux/Docker, 30s, 80/20 mix, threadpool (8 workers), 8 clients, writes against the elected leader: single-node baseline 18,481 ops/sec (p50=31µs p95=1948µs p99=2132µs) vs. 3-node cluster 9,141.6 ops/sec (p50=35µs p95=4045µs p99=5873µs) — ~2.0x lower throughput; p50 barely moves since the 80% GET share never touches Raft, but p95/p99 roughly double-to-triple from the 20% SET share now paying for majority-commit AppendEntries round trips to both followers. Raw: [raft-replication-2026-08-23.txt](benchmarks/results/raft-replication-2026-08-23.txt) |
| Failover recovery | 3-node Raft, leader killed | Failover time: leader `SIGKILL` to first successful write on the new leader | Linux/Docker, idle cluster (no concurrent write load), 3 fresh-cluster runs: 382ms, 275ms, 289ms — **median 289ms**, comfortably inside the randomized 250–400ms election timeout window (raft/node.cpp). Raw: [failover-recovery-2026-08-24.txt](benchmarks/results/failover-recovery-2026-08-24.txt) |
| Group commit | WAL group commit | Batching concurrent fsyncs to fix WAL serialization, the top measured write-path bottleneck | Linux/Docker, threadpool (8 workers), 8 clients. Baseline profile identified the bottleneck by direct measurement (see [optimization-baseline-2026-08-24.txt](benchmarks/results/optimization-baseline-2026-08-24.txt)). Single-node write-heavy (ratio 20) throughput **3.01x** (4,570 → 13,763 ops/sec); write-path scaling check flips from flat/declining with concurrency to actually scaling (**3.4x–3.65x** at 8–32 clients). Raw: [group-commit-2026-08-24.txt](benchmarks/results/group-commit-2026-08-24.txt) |
| Read confirm amortization | Skip the Raft quorum round trip on GET when a majority acked recently | Fixing the per-GET quorum round trip, the second measured bottleneck | Linux/Docker, threadpool (8 workers), 8 clients. Read-only Raft-leader throughput **12.2x** (12,102 → 151,044 ops/sec); 80/20 mix **1.53x** with p50 **20.6x** lower. Connection reuse was evaluated and not implemented — see [performance-notes.md](performance-notes.md) for why. Raw: [read-confirm-amortization-2026-08-24.txt](benchmarks/results/read-confirm-amortization-2026-08-24.txt) |
